    ...
    if ($status == 0) {
	    ...
	    $hr_min = DATE("g:i A", STRTOTIME($tmp[8]));
            ...
            if ((eregi('night', $dir) == 1) || (eregi('week', $dir) == 1) || (eregi('hour', $dir) == 1)) {
                if (eregi('hour', $dir) == 1) {
                    $options .= "<option title=\"WTF\" value=\"$oldhome/.snapshot/$dir/Maildir\">$date $hr_min</option>";
                }
		elseif ($today == $date) {
		  $options .= "<option title=\"WTF\" value=\"$oldhome/.snapshot/$dir/Maildir\">Today $hr_min</option>";
		}
                else {
                    $options .= "<option title=\"WTF\" value=\"$oldhome/.snapshot/$dir/Maildir\">$date</option>";
                }
            }
        }
    }
...
<div id="webtools-innerbox" class="ui-widget ui-widget-content ui-corner-all">
    <div style="margin-left: 10%; margin-right: 10%;">
        <form id="date_form" onsubmit="restore_post(); return false;">
            ...
            <select id="pathselect" name="path" style="width: 30em;">
                <?=$options?>
            </select>
